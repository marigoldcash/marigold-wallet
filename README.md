...
Download the one for your machine from the [latest release](https://github.com/marigoldcash/marigold-wallet/releases/latest). A release is named after the wallet inside it: v2.69.292 is what the front note shows and what `marigold-cli --version` prints, so you can always tell whether you are on the latest.
